...
data_name = "wdbc";
re_num = 140 ;  %区域数值
max_re = 0.10;   %相邻区域差值超过这个数，默认该区域为噪声区域，选择旁边区域加噪声区域
...
kPar = [10^-3,10^-2,10^-1,10^0,10^1,10^2,10^3];
...
for clc_index=1:clc
    ...
    for i=1:totalClass
        tra1{i} = segment{1,i};
        tra = [tra;tra1{i}];
        total = total + size(tra1{i},1);
    end
    sampleRateSet = selec_sample(tra1,re_num,max_re);
    if length(sampleRateSet)==1
        fprintf("重新输入一个区域数，这个区域数取的值有问题")
        return
    end

    
    k = get_kPar(sampleRateSet(:,1:end-1));
...
for kPara2_dex=1:length(kPar)
    kPar2 = kPar(kPara2_dex)*k;
for kPara3_dex=1:length(kPar)
    kPar3 = kPar(kPara3_dex)*k;
for RateId = 1 : 1
    ...
    res(totalCycle+5 , 1) = size(sampleRateSet,1)/total;
